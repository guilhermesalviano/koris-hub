...
Plugins são a forma de estender o Koris sem tocar no core. Há duas famílias de plugin —
[canais](/pt-br/docs/channels) e [ferramentas](/pt-br/docs/tools) — que compartilham o
mesmo formato e o mesmo kernel, descritos abaixo. A pasta `plugins/` também abriga
...
`PluginRegistry` e `buildRegistry`. As duas famílias se apoiam nele. Na inicialização,
`core/src/app.ts` monta um único `PluginRegistry` compartilhado a partir dos plugins de
canal e de ferramenta juntos.
...
`contracts.ts` da outra família.

O core depende dos SDKs e injeta serviços concretos no sentido contrário, por um
`PluginContext` (canais) ou `ToolPluginContext` (ferramentas) montado na raiz de
composição, `core/src/app.ts`. Cada pasta de plugin expõe uma função `create(context)`
que recebe esse contexto.
...
O scanner carrega todo subdiretório da pasta de uma família — `plugins/channels/` e
`plugins/tools/`. Arquivos soltos nessas pastas, como o `contracts.ts`, são ignorados.
Adicionar um plugin significa adicionar uma pasta; para ferramentas, o
`pnpm scaffold:tool <nome>` cria uma para você. Nenhuma alteração no core é necessária.
(`plugins/skills/` fica ao lado dessas duas, mas não é varrida aqui — veja
...
Plugins de canal continuam guardando seus segredos e a flag de confiança
`allow_unlisted_senders` no `config.yml` de cada pasta; somente a flag `enabled` foi para
o banco.
