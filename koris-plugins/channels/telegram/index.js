...
// ../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/client.js
var require_client = __commonJS({
  "../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/client.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.TelegramBot = void 0;
    ...
    var TelegramBot = class {
      ...
      isRetryableTelegramError(errorCode) {
        if (!errorCode) {
          return false;
        }
      ...
      isRetryableNetworkError(error) {
        ...
        return msg.includes("fetch failed") || msg.includes("aborted") || msg.includes("timeout") || msg.includes("etimedout") || msg.includes("econnreset") || msg.includes("eai_again") || msg.includes("enotfound");
      }
    };
    exports2.TelegramBot = TelegramBot;
  }
});

// ../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/bot.js
var require_bot = __commonJS({
  "../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/bot.js"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    exports2.initBot = initBot;
    exports2.getBot = getBot;
    exports2.resetBotForTests = resetBotForTests;
    var client_1 = require_client();
    var bot;
    function initBot(params) {
      const polling = params.polling ?? true;
      bot = new client_1.TelegramBot(params.token, { polling });
    ...
    function resetBotForTests() {
      bot = void 0;
    }
  }
});

// ../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/index.js
var require_dist = __commonJS({
  "../../../../projects/koris-hub/node_modules/.pnpm/@guilhermesalviano+telegram-bot@0.1.2/node_modules/@guilhermesalviano/telegram-bot/dist/index.js"(exports2) {
    "use strict";
    ...
    __exportStar(require_bot(), exports2);
  }
});
...
// artifacts/source/channels/telegram/index.ts
var index_exports = {};
__export(index_exports, {
  NOT_AUTHORIZED_MESSAGE: () => NOT_AUTHORIZED_MESSAGE,
  TelegramChannel: () => TelegramChannel,
  TelegramChannelFactory: () => TelegramChannelFactory,
  _setBotUsernameForTesting: () => _setBotUsernameForTesting,
  _setTelegramWhitelistForTesting: () => _setTelegramWhitelistForTesting,
  configureTelegramRuntime: () => configureTelegramRuntime,
  create: () => create,
  createTelegramPlugin: () => createTelegramPlugin,
  handleMessage: () => handleMessage,
  liveChannel: () => liveChannel,
  loadTelegramConfig: () => loadTelegramConfig,
  sendCode: () => sendCode,
  sendText: () => sendText,
  sendWithApproval: () => sendWithApproval,
  writeTelegramConfigPatch: () => writeTelegramConfigPatch
});
...
// plugins/channels/contracts.ts
var ADAPTERS = new ExtensionPoint("channels.adapters");
...
// artifacts/source/channels/telegram/constants.ts
var NOT_AUTHORIZED_MESSAGE = "You're not authorized to send messages here yet. Please ask the administrator to add you to the allowed list.";
var TYPING_INTERVAL_MS = 5e3;
var TELEGRAM_MESSAGE_LIMIT = 4e3;
var TELEGRAM_CAPABILITIES = {
  streaming: true,
  markdown: true,
  interactive: false,
...
// artifacts/source/channels/telegram/mention.ts
function isBotMentioned(text, entities, username) {
...
// artifacts/source/channels/telegram/state.ts
var telegramState = {
  botUsername: null,
  botToken: "",
  channelHandler: void 0,
  telegramWhitelist: /* @__PURE__ */ new Set(),
  allowUntrusted: false
};
function _setBotUsernameForTesting(username) {
  telegramState.botUsername = username;
}
function _setTelegramWhitelistForTesting(ids) {
  telegramState.telegramWhitelist.clear();
  ids.forEach((id) => telegramState.telegramWhitelist.add(id));
}

// artifacts/source/channels/telegram/media.ts
...
function telegramFileDownloadUrl() {
  return `https://api.telegram.org/file/bot${telegramState.botToken}`;
}

// artifacts/source/channels/telegram/channel.ts
var TelegramChannel = class {
  constructor(bot) {
    this.bot = bot;
  }
  async handleMessage(gateway, msg) {
    const { id: chatId, type: chatType } = msg.chat;
    const telegramMsg = msg;
    ...
    if (quotedPhoto) {
      ...
      images.push(...quotedImages.map((img) => ({ ...img, source: "quoted" })));
    }
...
\`\`\``, { parse_mode: "Markdown" });
  ...
  async sendWithApproval(logger, chatId, message, callbackData) {
  ...
  async processAndReply(gateway, chatId, text, images, isGroup, mentionsBot, isTrustedSender, groupName, quotedText) {
    try {
      await this.withTypingIndicator(chatId, async () => {
        const handler = telegramState.channelHandler.create({
          ...
          reply: {
            sendText: (target, reply) => this.sendText(Number(target), reply),
  ...
  async downloadPhoto(fileId) {
    try {
      ...
      const filePath = fileData.ok ? fileData.result?.file_path : void 0;
      if (!filePath) {
        return [];
      }
  ...
  async sendMessageWithMarkdownFallback(chatId, text) {
  ...
  async withTypingIndicator(chatId, work) {
    try {
  ...
  async getBotClient() {
    ...
    const { getBot } = await Promise.resolve().then(() => __toESM(require_dist()));
    return getBot();
  }
};

// artifacts/source/channels/telegram/factory.ts
var TelegramChannelFactory = class {
  static create() {
    return new TelegramChannel();
  }
  static async start(options) {
    const channel = new TelegramChannel();
    const { initBot } = await Promise.resolve().then(() => __toESM(require_dist()));
    const bot = initBot({
      token: options.token,
      polling: true,
      onMessage: (msg) => {
    ...
    }).catch((err) => options.logger.warn(`Failed to fetch bot info: ${err.message}`));
    return {
      channel,
...
// artifacts/source/channels/telegram/adapter.ts
function createTelegramAdapter(options) {
  return {
    name: "telegram",
    enabled: () => options.isEnabled() && options.token.length > 0,
    capabilities: TELEGRAM_CAPABILITIES,
    start: (logger, gateway) => {
      let stopFn = null;
      TelegramChannelFactory.start({ token: options.token, gateway, logger }).then(({ stop }) => {
        stopFn = stop;
      }).catch((err) => logger.warn(`Failed to start Telegram bot: ${err.message}`));
      return () => {
        stopFn?.();
      };
    },
    sendMessage: async (_logger, target, message) => {
...
function createTelegramPlugin(options) {
  return {
    name: "telegram",
    setup(registry) {
...
// plugins/config/loader.ts
var import_fs = require("fs");
var import_path = require("path");
var import_yaml = __toESM(require_dist2());
var defaultFileIO = {
  exists: import_fs.existsSync,
  read: (path) => (0, import_fs.readFileSync)(path, "utf-8")
};
function loadPluginConfigFile(options) {
  const { pluginDir, filename = "config.yml" } = options;
  const fileIO = options.fileIO ?? defaultFileIO;
  const path = (0, import_path.normalize)((0, import_path.join)(pluginDir, filename));
  if (!fileIO.exists(path)) {
    return {};
  }
  try {
    const parsed = (0, import_yaml.parse)(fileIO.read(path));
...
function resolvePluginDir(pluginName, options = {}) {
  const cwd = options.cwd ?? process.cwd();
  const exists = options.exists ?? import_fs.existsSync;
  ...
  const dataDirCandidate = dataDir ? (0, import_path.normalize)((0, import_path.join)(dataDir, "plugins", family, pluginName)) : void 0;
  const candidates = [
    ...dataDirCandidate ? [dataDirCandidate] : [],
    (0, import_path.join)(cwd, "plugins", family, pluginName),
    (0, import_path.join)(cwd, "apps", "client", "plugins", family, pluginName)
  ].map((candidate) => (0, import_path.normalize)(candidate));
  const found = candidates.find((candidate) => exists((0, import_path.join)(candidate, filename)));
  if (found) {
    return found;
  }
  if (dataDirCandidate) {
  ...
  const repoFamilyDir = (0, import_path.normalize)((0, import_path.join)(cwd, "plugins", family));
  if (exists(repoFamilyDir)) {
    return (0, import_path.normalize)((0, import_path.join)(repoFamilyDir, pluginName));
  }
  return options.fallbackDir ? (0, import_path.normalize)(options.fallbackDir) : candidates[0];
}
function getPluginConfigValue(yamlKey, fallback, yamlConfig, envKey, env = process.env) {
...
function mergePluginConfigPatch(base, patch) {
  const result = { ...base };
  for (const [key, patchValue] of Object.entries(patch)) {
    const baseValue = result[key];
    result[key] = isPlainObject(patchValue) && isPlainObject(baseValue) ? mergePluginConfigPatch(baseValue, patchValue) : patchValue;
  }
  return result;
}
function writePluginConfigPatch(patch, options) {
  const { pluginDir, filename = "config.yml" } = options;
  const exists = options.exists ?? import_fs2.existsSync;
  const readFile = options.readFile ?? ((path2) => (0, import_fs2.readFileSync)(path2, "utf-8"));
  const path = (0, import_path2.normalize)((0, import_path2.join)(pluginDir, filename));
  let current = {};
  if (exists(path)) {
    try {
      const parsed = (0, import_yaml2.parse)(readFile(path));
  ...
  const content = (0, import_yaml2.stringify)(merged);
...
// plugins/config/define-config.ts
function definePluginConfig(definition) {
...
// plugins/channels/channel-config.ts
function defineChannelConfig(definition) {
  return definePluginConfig({ ...definition, family: "channels" });
}

// artifacts/source/channels/telegram/config.ts
var telegramConfig = defineChannelConfig({
  pluginName: "telegram",
...
var loadTelegramConfig = telegramConfig.load;
var writeTelegramConfigPatch = telegramConfig.writePatch;

// artifacts/source/channels/telegram/runtime.ts
function configureTelegramRuntime(cfg) {
...
function create(context, configOverride) {
  const cfg = configOverride ?? loadTelegramConfig();
  if (context.pluginEnablement.isEnabled("telegram") && !cfg.token) {
    context.logger.warn(
      "[telegram] enabled but bot_token is empty \u2014 the channel will not start. Set CHANNELS_TELEGRAM_BOT_TOKEN or bot_token in plugins/channels/telegram/config.yml."
    );
  }
